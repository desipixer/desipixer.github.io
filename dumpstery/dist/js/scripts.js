...
app.service('service.util', ['$http', 'service.auth', '$q', function ($http, authService, $q) {
    ...
    try {
        fetch('./dist/files/actress.json').then(function (response) {
...
app.service('service.blog', ['service.auth', '$q', function (authService, $q) {
    var key = authService.getKey();
    var blogItems = [];
    var maxItems = 500;

    //use axios service.
    function getAllItems(blogId = "7833828309523986982", nextToken, itemArray = [], currentItem = 1, totalItems = 0 , deferred) {
        if (currentItem >= totalItems) {
            return itemArray;
        } 
        var feedAPIUrl = getBlogUrl(blogId, maxItems, nextToken);

        axios.get(feedAPIUrl).then(function (res) {
            console.log("axios : data, ", res.data);
            currentItem = currentItem + maxItems;
            itemArray = itemArray.concat(res.data.items);
            if(currentItem < totalItems){
                getAllItems(blogId, res.data.nextPageToken, itemArray, currentItem, totalItems, deferred);
            } else {
    ...
    function getBlogProperty(blogName) {
        if (blogName) {
            return axios.get(getBlogIdFromUrl(blogName));
        } else {
            console.log("ERROR >> blogName is invalid : ", blogName);
        }

    }


    function getBlogIdFromUrl(blogName = "https://desipixer.blogspot.com") {
        try {
            var myUrlObj = new URL(blogName);
            var hostname = myUrlObj.origin;
            return `https://www.googleapis.com/blogger/v3/blogs/byurl?key=${key}&url=${hostname}`;
        } catch (ex) {
            throw ex;
        }
    }
    ...
    function getBlogUrl(blogId = '', maxResults = 500, nextToken) {
        var returnUrl = `https://www.googleapis.com/blogger/v3/blogs/${blogId}/posts?fetchImages=true&key=${key}&maxResults=${maxResults}`;
        if (nextToken) {
            returnUrl = returnUrl.concat(`&pageToken=${nextToken}`)
        }
        return returnUrl;
    }
 ...
 */
app.controller('myCtrl', ['$scope', '$http', 'service.util', '$q', 'service.auth', 'service.wp', 'service.blog',
    function ($scope, $http, serviceUtil, $q, authService, wpService, blogService) {
        ...
        var blogName = 'https://glam-desi.blogspot.com/';
        $scope.blogName = blogName;

        $scope.postDataLoaded = false;
        $scope.postData = [];
        $scope.postDataLength = 0;
        $scope.postDataCleaned = [];

        $scope.getNgAllItems = function () {
            // get blogname
            console.log('blogname : ', $scope.blogName);
            var blogName = $scope.blogName;
            blogService.getBlogProperty(blogName).then(function (res) {
                console.log(res.data);
                var data = res.data;
                var totalItems = data.posts.totalItems;
                var id = data.id;
                if (totalItems) {
                    try {
                        var deferred = $q.defer();
        ...
        //blogService.getAllItems('7833828309523986982', null, [], 1, 500);
        ...
        /**
         * since we post the data from site directly, we don't need uploader.
         * need to process data again.
         * 
         */
        ...
        function dumpImages(arr) {
            if (arr) {
                try {
                    ...
                    postDumpImages(arr, start, end, count, errCount, 1);
                } catch (ex) {
        ...
        function postDumpImages(arr, start, end, count, errCount, imageCount = 1) {
        ...
        $scope.onConfigChanged = function () {
            ...
            var fileSelect = "dist/config/" + thisConfig;
